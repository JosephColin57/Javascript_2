...
//        1. Obtener una lista de los géneros sin repetir
    ...
    const genre = (dataArray) => {
        const genreMovie = new Set()
      
        dataArray.forEach(data => {
            genreMovie.add(data.genre)
        })
      
        return Array.from(genreMovie)
      }
    ...
    const director = (dataArray) => {
        const directorMovie = new Set()
      
        dataArray.forEach(data => {
            directorMovie.add(data.director)
        })
      
        return Array.from(directorMovie)
      }
    ...
    const protagonists = (dataArray) => {
        return Array.from(dataArray.reduce((protagonistsList, movie) => {
          const actorNames = movie.protagonists.map(actor => actor.name);
          const combinedList = [...protagonistsList, ...actorNames];
          const uniqueActors = new Set(combinedList);
          console.log("Lista combinada de nombres de actores:", combinedList);
          console.log("Lista de nombres de actores únicos:", Array.from(uniqueActors));
          return uniqueActors;
        }, new Set()));
      };
      
      console.log(protagonists(movies));

    //        4. Obtener uan lista de clasificaciones (rating) sin repetir

    const rating = (dataArray) => {
        return dataArray.reduce((ratings, data) => {
          return new Set([...ratings, data.rating])
        }, new Set())
      }
...
/* ¡Claro! Aquí tienes la explicación paso a paso de lo que hace la función `peliculasEnRango`:

1. **Parámetros de entrada**: La función `peliculasEnRango` toma tres parámetros de entrada:
   - `dataArray`: La matriz de datos de películas en la que se buscarán las películas en el rango de duración.
   - `duracionMinima`: El límite inferior del rango de duración deseado para las películas.
   - `duracionMaxima`: El límite superior del rango de duración deseado para las películas.

2. **Filtrado con `filter()`:** Se utiliza el método `filter()` en la matriz `dataArray` para crear una nueva matriz que contendrá solo las películas que cumplan con la condición especificada.

3. **Iteración sobre las películas:** Para cada película en `dataArray`, se ejecuta una función de filtro que verifica si la duración de la película está dentro del rango especificado.

4. **Extracción y conversión de duración:** Se extrae la duración de la película, que inicialmente está en formato de cadena (por ejemplo, "120 minutos"). Se utiliza el método `split()` para dividir la cadena en un array, separando el número de minutos del texto "minutos". Luego, se utiliza `parseInt()` para convertir el número de minutos en un entero.

5. **Comparación de duración:** Se compara la duración de la película con los valores de `duracionMinima` y `duracionMaxima` para determinar si la película está dentro del rango deseado.

6. **Creación de la lista resultante:** Todas las películas que cumplen con la condición de duración dentro del rango especificado se incluyen en la nueva matriz resultante.

7. **Retorno del resultado:** La función devuelve la nueva matriz que contiene solo las películas que tienen una duración dentro del rango especificado.
