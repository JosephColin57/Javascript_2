...
const createBlogCard = (entryObject) => {
  let { image, title, abstract, avatar, autor } = entryObject;
  ...
  cardAutor.classList.add("card", "blog-card-avatar", "mb-3", "d-flex");

  let cardImageAutor = document.createElement("img");
  cardImageAutor.setAttribute("src", avatar);
  cardImageAutor.classList.add("mr-3")

  let cardBodyAutor = document.createElement("div");
  cardBodyAutor.classList.add("card-body");

  let cardTitleAutor = document.createElement("h3");
  cardTitleAutor.classList.add("card-title");
  let cardTitleTextAutor = document.createTextNode(autor);
  cardTitleAutor.append(cardTitleTextAutor);

  cardBodyAutor.append(cardTitleAutor);
  cardAutor.append(cardImageAutor, cardBodyAutor);

  cardBody.append(cardTitle, cardText);
  card.append(cardImage, cardBody, cardAutor);
...
const authorName = (entryObject) => {
  let {autor, avatar} = entryObject;

  let cardautor = document.createElement("div");
  cardautor.classList.add("card", "blog-card", "mb-3");

  let cardImageAutor = document.createElement("img");
  cardImageAutor.setAttribute("src", avatar);
  cardImageAutor.classList.add("rounded-circle", "rounded-5", "size")
...
const container = document.getElementById("authorCardsContainer");

const uniqueAuthors = blogEntries.reduce((acc, entry) => {
  if (!acc[entry.autor]) {
    acc[entry.autor] = authorName(entry);
  }
  return acc;
}, {});

const uniqueAuthorCards = Object.values(uniqueAuthors);

uniqueAuthorCards.forEach(authorCard => {
